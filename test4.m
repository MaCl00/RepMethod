...
% x^2*H_x.
syms x;
functions = {factorial(sym(x)), 2^sym(x), 10000000*(psi(sym(x)+1)- psi(sym(1)))};
precision = 30; 
start = 0;
rec_degree = 3;
poly_degree = 4;
solution = repertoire(functions, @substitute, 0, precision, start, rec_degree, poly_degree, true, false);
