function [M, function_name] = generateGuessesModified(func_guess, range, num_poly, verbose)
    syms x;
    ...
    for j=1:size(M, 1)
        for i=1:size(polyMatrix, 1)
            c = M(j, i);
            M(j, :) = M(j, :) - c*polyMatrix(i, :);
        end
        M(j, :) = M(j, :) ./ M(j, size(polyMatrix, 1)+1);
    end
    if verbose
    ...
    for i = 1:(current_row-1)
        for j= 2:num_poly
            M(current_row, (size(polyMatrix, 1)+1):end) = M(i, (size(polyMatrix, 1)+1):end) .* polyMatrix(j, 1:(end-size(polyMatrix, 1)));
            current_row = current_row + 1;
    ...
    M = [M(current_row:current_row + num_poly - 1, :); M(1:current_row-1, :);];
    for i=1:size(M, 1)
        gcdRow = gcd(M(i, i:end));
        M(i, :) = M(i, :) ./ gcdRow;
    end
