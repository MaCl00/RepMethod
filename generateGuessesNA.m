function [M, function_name] = generateGuessesNA(func_guess, range, num_poly, verbose)
    syms x;
    num_func = length(func_guess);
    if verbose
        fprintf('Evaluating function-guesses');
    end
    sequences = cellfun(@(j) cellfun(@(i) subs(func_guess{j}, x, i), range), num2cell(1:num_func), 'UniformOutput', false);
    if verbose
    ...
    for j=1:num_poly-1
        polynomes{j+1} = cellfun(@(i) polynomes{j}(i- range{1}(1) + 1) * (i - j + 1 - range{1}(1)) / j, range);
